...
class visualize:
    ...
    def chart_init(self):
        ...
        self.line_window = self.viz.line(Y=np.array([0]), X=np.array([0]), opts=dict(title = "dqn - reward value", xlabel='Episode', ylabel='Reward'))
        ...
        self.Learning_curve = self.viz.line(Y=np.array([0]), X=np.array([0]), opts=dict(title = "dqn - Learning Curve", xlabel='Episode', ylabel='Steps / Episode'))
    ...
    def learning_curve_update(self, episode, steps):
        SpE = steps / episode
        self.viz.line(Y=np.array([SpE]), X=np.array([episode]), win=self.Learning_curve, update='append')
